...
class FixedCapBiMap {
	...
	private:
		TElem* elements;
		int capacity; //maximum nr of pairs
		int number_of_pairs;
	...
	//removes a key k with all the values that belong to it. Returns a ValuePair containing the removed values in the
	//following way : - if k has 2 associated values, these are in valuePair
	//                - if k had 1 associated value, that value is the first component of the ValuePair, the second
	//                  component is NULL_TVALUE
	//                - if k had no associated value, both components are NULL_TVALUE
